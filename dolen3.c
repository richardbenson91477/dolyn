...
int load_quantized_qwen3(const char *filepath, Qwen3 *model_qwen3, int seq_n_max) {
    ...
    if (!w->wq || !w->wk || !w->wv || !w->wo || !w->w1 || !w->w2 || !w->w3) {
        fprintf(stderr, "Failed to allocate memory for quantized tensors\n");
        fclose(f);
        return -1;
    }
