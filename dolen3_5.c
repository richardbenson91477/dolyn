...
int load_quantized_qwen3_5(const char *filepath, Qwen3_5 *model_qwen3_5, int seq_n_max) {
    ...
    if (!model_qwen3_5->layer_types || !model_qwen3_5->attn_layer_indices || 
        ...
        fprintf(stderr, "Failed to allocate memory for layer indices\n");
        fclose(f);
        return -1;
    }
    
    if (fread(model_qwen3_5->layer_types, sizeof(int), (size_t)p->n_layer, f) != (size_t)p->n_layer) {
        fprintf(stderr, "Failed to read layer_types from %s\n", filepath);
        fclose(f);
        return -1;
    }
    ...
    if (!w->rms_att_weight) {
        fprintf(stderr, "Failed to allocate rms_att_weight\n");
        fclose(f);
        return -1;
    }
    if (fread(w->rms_att_weight, sizeof(float), (size_t)p->n_layer * p->dim, f) != (size_t)p->n_layer * p->dim) {
        fprintf(stderr, "Failed to read rms_att_weight\n");
        fclose(f);
        return -1;
    }
    ...
    if (p->n_linear_attn_layers > 0) {
        ...
        if (!w->in_proj_qkv || !w->in_proj_z) {
            fprintf(stderr, "Failed to allocate linear attention projections\n");
            fclose(f);
            return -1;
        }
        ...
        if (!w->in_proj_b || !w->in_proj_a || !w->conv1d_weight || !w->dt_bias || !w->A_log || !w->linear_norm) {
            fprintf(stderr, "Failed to allocate linear attention weights\n");
            fclose(f);
            return -1;
        }
        ...
        if (fread(w->conv1d_weight, sizeof(float), (size_t)p->n_linear_attn_layers * conv_dim * p->linear_conv_kernel, f) != 
            ...
            fprintf(stderr, "Failed to read conv1d_weight\n");
            fclose(f);
            return -1;
        }
        ...
        if (fread(w->linear_norm, sizeof(float), (size_t)p->n_linear_attn_layers * p->d_linear_v, f) != 
            ...
            fprintf(stderr, "Failed to read linear_norm\n");
            fclose(f);
            return -1;
        }
        ...
        if (!w->out_proj) {
            fprintf(stderr, "Failed to allocate out_proj\n");
            fclose(f);
            return -1;
        }
        for (int i = 0; i < p->n_linear_attn_layers; i++) {
    ...
    if (!w->rms_ffn_weight) {
        fprintf(stderr, "Failed to allocate rms_ffn_weight\n");
        fclose(f);
        return -1;
    }
    if (fread(w->rms_ffn_weight, sizeof(float), (size_t)p->n_layer * p->dim, f) != (size_t)p->n_layer * p->dim) {
        fprintf(stderr, "Failed to read rms_ffn_weight\n");
        fclose(f);
        return -1;
    }
    ...
    if (!w->w1 || !w->w2 || !w->w3) {
        fprintf(stderr, "Failed to allocate MLP weights\n");
        fclose(f);
        return -1;
    }
    for (int i = 0; i < p->n_layer; i++) {
    ...
    if (!w->rms_final_weight) {
        fprintf(stderr, "Failed to allocate rms_final_weight\n");
        fclose(f);
        return -1;
    }
    if (fread(w->rms_final_weight, sizeof(float), (size_t)p->dim, f) != (size_t)p->dim) {
        fprintf(stderr, "Failed to read rms_final_weight\n");
        fclose(f);
        return -1;
    }
    ...
    fclose(f);
    fprintf(stderr, "Quantized model loaded from %s\n", filepath);
