...
static int write_layer_f32(quantize_ctx *ctx, FILE *out, int layer,
    ...
    snprintf(name, sizeof(name),
            "model.language_model.layers.%d.%s", layer, suffix);
...
int quantize_q3_5_to_file(const char *model_dir, const char *output_file) {
    ...
    if (quantize_write_bytes(out, &magic, sizeof(magic), 1) ||
            ...
            quantize_write_qtensor(&ctx, out,
    ...
    for (int l = 0; l < p->n_layer; l++) {
        if (model.layer_types[l] == 0 &&
                write_layer_f32(&ctx, out, l,
                    "self_attn.q_norm.weight", head_size)) {
            failed = 1; goto cleanup;
        }
    }
    for (int l = 0; l < p->n_layer; l++) {
        if (model.layer_types[l] == 0 &&
                write_layer_f32(&ctx, out, l,
                    "self_attn.k_norm.weight", head_size)) {
            failed = 1; goto cleanup;
        }
    }
    ...
    if (p->n_linear_attn_layers > 0) {
        for (int l = 0; l < p->n_layer; l++) {
            ...
            if (write_layer_qt(&ctx, out, l,
                        "linear_attn.in_proj_qkv.weight", conv_dim, p->dim) ||
                    write_layer_qt(&ctx, out, l,
                        "linear_attn.in_proj_z.weight", value_dim, p->dim)) {
                failed = 1; goto cleanup;
            }
        }
        for (int l = 0; l < p->n_layer; l++) {
        ...
        for (int l = 0; l < p->n_layer; l++) {
            if (model.layer_types[l] == 1 &&
                    write_layer_f32(&ctx, out, l,
                        "linear_attn.dt_bias", p->n_linear_v_heads)) {
                failed = 1; goto cleanup;
            }
        }
        for (int l = 0; l < p->n_layer; l++) {
            if (model.layer_types[l] == 1 &&
                    write_layer_f32(&ctx, out, l,
                        "linear_attn.A_log", p->n_linear_v_heads)) {
                failed = 1; goto cleanup;
            }
        }
        for (int l = 0; l < p->n_layer; l++) {
            if (model.layer_types[l] == 1 &&
                    write_layer_f32(&ctx, out, l,
                        "linear_attn.norm.weight", p->d_linear_v)) {
                failed = 1; goto cleanup;
            }
        }
        for (int l = 0; l < p->n_layer; l++) {
            if (model.layer_types[l] == 1 &&
                    write_layer_qt(&ctx, out, l,
                        "linear_attn.out_proj.weight", p->dim, value_dim)) {
                failed = 1; goto cleanup;
            }
        }
    }
    ...
    for (int l = 0; l < p->n_layer; l++) {
        if (write_layer_f32(&ctx, out, l,
                    "post_attention_layernorm.weight", p->dim)) {
            failed = 1; goto cleanup;
        }
    }
