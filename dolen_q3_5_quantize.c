...
int quantize_q3_5_to_file(
    ...
    if (quantize_write_bytes(out, model.layer_types, sizeof(int), p->n_layer)) {
        failed = 1;
        goto cleanup;
    }

    if (quantize_write_tensor(&ctx, out, "model.language_model.embed_tokens.weight",
...
int main(int argc, char *argv[]) {
    ...
    return quantize_q3_5_to_file(argv[1], argv[2], embed_type, attn_type, mlp_type, tokenizer_path) ? EXIT_FAILURE : EXIT_SUCCESS;
}
