...
void alloc_state_gemma4u(state_gemma4u *s, config_gemma4u *p) {
    ...
    if (rotary_full > 0) {
        ...
        s->sin_cache_full = (float *)a_calloc((size_t)p->seq_len * rotary_full * sizeof(float));
        for (int pos = 0; pos < p->seq_len; pos++) {
            for (int i = 0; i < rotary_full; i++) {
                float freq = 1.0f / powf(p->rope_theta_full, (float)(2 * i) / p->global_head_dim);
                float val = pos * freq;
                s->cos_cache_full[pos * rotary_full + i] = cosf(val);
    ...
    if (rotary_sliding > 0) {
        ...
        s->sin_cache_sliding = (float *)a_calloc((size_t)p->seq_len * rotary_sliding * sizeof(float));
        for (int pos = 0; pos < p->seq_len; pos++) {
            for (int i = 0; i < rotary_sliding; i++) {
                float freq = 1.0f / powf(p->rope_theta_sliding, (float)(2 * i) / p->head_dim);
                float val = pos * freq;
                s->cos_cache_sliding[pos * rotary_sliding + i] = cosf(val);
