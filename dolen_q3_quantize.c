...
int quantize_q3_to_file(const char *model_dir, const char *output_file) {
    ...
    for (int l = 0; l < config.n_layers; l++) {
        if (write_layer_tensor(&ctx, out, l, "self_attn.q_norm.weight",
    ...
    for (int l = 0; l < config.n_layers; l++) {
        if (write_layer_tensor(&ctx, out, l, "self_attn.k_norm.weight",
                    1, head_size, Q_TYPE_F32)) {
            failed = 1;
            goto cleanup;
        }
    ...
    for (int l = 0; l < config.n_layers; l++) {
        if (write_layer_tensor(&ctx, out, l, "self_attn.q_proj.weight",
                    all_heads_dim, config.dim, Q_TYPE_Q8) ||
                ...
                write_layer_tensor(&ctx, out, l, "self_attn.o_proj.weight",
                    config.dim, all_heads_dim, Q_TYPE_Q8) ||
                write_layer_tensor(&ctx, out, l, "mlp.gate_proj.weight",
                    config.hidden_dim, config.dim, Q_TYPE_Q8) ||
                write_layer_tensor(&ctx, out, l, "mlp.down_proj.weight",
                    config.dim, config.hidden_dim, Q_TYPE_Q8) ||
