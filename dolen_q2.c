...
int load_quantized_q2(const char *_file_path_s, Q2 *_model, int seq_n_max) {
    ...
    alloc_state_q2(&(_model->state), &(_model->config));

    return 0;
}

float *forward_q2(Q2 *_model, int token, int pos) {
    config_q2 *_config = &_model->config;
    weights_q2 *_weights = &_model->weights;
...
static float *forward_q2_wrap(void *_model, int token, int pos) {
...
model_iface *init_q2(const char *_model_path_s, int seq_n_max, bool think_) {
    Q2 *_model = a_calloc(1 * sizeof(Q2));

    if (load_quantized_q2(_model_path_s, _model, seq_n_max)) {
        free_q2(_model);
        free(_model);
        return NULL;
    }
