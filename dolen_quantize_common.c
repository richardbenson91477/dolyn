...
static size_t dtype_size(st_dtype dtype) {
    switch (dtype) {
        case ST_DTYPE_F16:
        case ST_DTYPE_BF16: return sizeof(uint16_t);
        case ST_DTYPE_F32: return sizeof(float);
        default: return 0;
    }
}

static st_dtype parse_dtype(const char *dtype) {
    if (!dtype) return ST_DTYPE_UNSUPPORTED;
    if (strcmp(dtype, "F16") == 0) return ST_DTYPE_F16;
    if (strcmp(dtype, "BF16") == 0) return ST_DTYPE_BF16;
    if (strcmp(dtype, "F32") == 0) return ST_DTYPE_F32;
    return ST_DTYPE_UNSUPPORTED;
}

static int json_u64(JsonValue *value, uint64_t *out) {
    if (!value || value->type != JSON_NUMBER || value->data.number < 0.0) return -1;
...
static weightmap_entry *find_index_entry(safetensors_idx *idx, const char *filename, const char *tensor_name) {
    for (size_t i = 0; i < idx->n_entries; i++) {
        ...
        if (strcmp(entry->filename, filename) == 0 && strcmp(entry->tensor_name, tensor_name) == 0) return entry;
    }
    return NULL;
}
...
static int load_shard_metadata(safetensors_idx *idx, const char *filename) {
    ...
    if (header_len_u64 == 0 || header_len_u64 > SIZE_MAX - 1) {
    ...
    for (size_t i = 0; i < root->data.object.count; i++) {
        ...
        if (strcmp(pair->key, "__metadata__") == 0) continue;
...
int load_safetensors_index(safetensors_idx *idx, const char *model_dir) {
    ...
    if (snprintf(index_path, sizeof(index_path), "%s/model.safetensors.index.json", model_dir) >= (int)sizeof(index_path)) return -1;
    ...
    if (file_size < 0 || fseeko(f, 0, SEEK_SET) != 0) { fclose(f); return -1; }
    ...
    for (size_t i = 0; i < idx->n_entries; i++) {
        ...
        for (int j = 0; j < idx->n_unique_files; j++) {
            if (strcmp(idx->unique_filenames[j], pair->value->data.string) == 0) {
                found = 1;
                break;
            }
        }
    ...
    for (int file_i = 0; file_i < idx->n_unique_files; file_i++) {
        for (size_t i = 0; i < idx->n_entries; i++) {
            if (strcmp(idx->entries[i].filename, idx->unique_filenames[file_i]) == 0) {
...
void quantize_ctx_close(quantize_ctx *ctx) {
    if (!ctx) return;
    if (ctx->source) fclose(ctx->source);
...
const weightmap_entry *quantize_find_tensor(const quantize_ctx *ctx, const char *name) {
    for (size_t i = 0; i < ctx->index.n_entries; i++) {
        if (strcmp(ctx->index.entries[i].tensor_name, name) == 0) return &ctx->index.entries[i];
    }
    return NULL;
}
...
const weightmap_entry *quantize_find_last_tensor(const quantize_ctx *ctx, const char *const *names, size_t n_names) {
    ...
    for (size_t n = 0; n < n_names; n++) {
        ...
        if (entry && (!best || entry->processing_rank > best->processing_rank)) best = entry;
    }
    return best;
}

static int open_entry_source(quantize_ctx *ctx, const weightmap_entry *entry) {
    if (!entry || !entry->metadata_ready) return -1;
    if (ctx->source && ctx->source_filename && strcmp(ctx->source_filename, entry->filename) == 0) return 0;
    ...
    char filepath[PATH_MAX];
    if (snprintf(filepath, sizeof(filepath), "%s/%s", ctx->index.model_dir, entry->filename) >= (int)sizeof(filepath)) return -1;
...
static int read_f32_range(quantize_ctx *ctx, const weightmap_entry *entry, uint64_t first_element, size_t elements,
    ...
    if (entry->dtype == ST_DTYPE_F32) {
        ...
        for (size_t i = 0; i < elements; i++) f32[i] = csafetensors_f16_to_f32(src[i]);
    } else if (entry->dtype == ST_DTYPE_BF16) {
        const uint16_t *src = (const uint16_t *)raw;
        for (size_t i = 0; i < elements; i++) f32[i] = csafetensors_bf16_to_f32(src[i]);
    } else {
        return -1;
    }
    return 0;
...
int quantize_write_tensor_entry(quantize_ctx *ctx, FILE *out, const weightmap_entry *entry, int rows, int cols, q_type_t type) {
    ...
    if (quantize_write_bytes(out, &type, sizeof(type), 1) ||
        quantize_write_bytes(out, &rows, sizeof(rows), 1) ||
    ...
    if (type == Q_TYPE_F32) {
        ...
        if (chunk_elements > expected_elements && expected_elements > 0) chunk_elements = expected_elements;

        size_t raw_bytes;
        if (checked_mul_size(chunk_elements, item_size, &raw_bytes)) return -1;
        void *raw = a_calloc(raw_bytes);
        float *f32 = (float *)a_calloc(chunk_elements * sizeof(float));
        if ((expected_elements > 0) && (!raw || !f32)) { free(raw); free(f32); return -1; }

        for (size_t done = 0; done < expected_elements;) {
            size_t n = expected_elements - done;
            if (n > chunk_elements) n = chunk_elements;
            if (read_f32_range(ctx, entry, done, n, raw, f32) || quantize_write_bytes(out, f32, sizeof(float), n)) {
                free(raw); free(f32); return -1;
            }
            done += n;
        }
        free(raw); free(f32);
        ...
        if ((expected_elements > 0) && (!raw || !f32 || !f16)) { free(raw); free(f32); free(f16); return -1; }

        for (size_t done = 0; done < expected_elements;) {
            size_t n = expected_elements - done;
            if (n > chunk_elements) n = chunk_elements;
            if (read_f32_range(ctx, entry, done, n, raw, f32)) {
                free(raw); free(f32); free(f16); return -1;
            }
            for (size_t i = 0; i < n; i++) f16[i] = (_Float16)f32[i];
            if (quantize_write_bytes(out, f16, sizeof(_Float16), n)) {
                free(raw); free(f32); free(f16); return -1;
            }
            done += n;
        }
        free(raw); free(f32); free(f16);
        return 0;
        ...
        if (checked_add_u64(q_offset, q_bytes, &s_offset) || checked_add_u64(s_offset, s_bytes, &end_offset)) return -1;

        if (end_offset > 0) {
            if (seek_abs(out, end_offset - 1) || fputc(0, out) == EOF) return -1;
        }

        size_t item_size = dtype_size(entry->dtype);
        size_t row_work_bytes = (size_t)cols * (item_size + sizeof(float) + sizeof(int8_t)) + (size_t)num_groups * sizeof(float);
        size_t rows_per_chunk = row_work_bytes ? ctx->chunk_bytes / row_work_bytes : 1;
        if (rows_per_chunk == 0) rows_per_chunk = 1;
        if (rows_per_chunk > (size_t)rows) rows_per_chunk = (size_t)rows;

        size_t max_elements;
        if (checked_mul_size(rows_per_chunk, (size_t)cols, &max_elements)) return -1;
        size_t raw_bytes;
        if (checked_mul_size(max_elements, item_size, &raw_bytes)) return -1;
        ...
        if (!raw || !f32 || !q || !s) { free(raw); free(f32); free(q); free(s); return -1; }

        for (int row = 0; row < rows;) {
            int chunk_rows = rows - row;
            if ((size_t)chunk_rows > rows_per_chunk) chunk_rows = (int)rows_per_chunk;
            size_t chunk_elements = (size_t)chunk_rows * cols;

            if (read_f32_range(ctx, entry, (uint64_t)row * cols, chunk_elements, raw, f32)) {
                free(raw); free(f32); free(q); free(s); return -1;
            }
...
int quantize_write_tensor_or_empty(quantize_ctx *ctx, FILE *out, const char *name, int rows, int cols, q_type_t type) {
    ...
    if (!entry) return quantize_write_empty_tensor(out);
...
int quantize_write_empty_tensor(FILE *out) {
    ...
    int zero = 0;
    return quantize_write_bytes(out, &type, sizeof(type), 1) ||
           quantize_write_bytes(out, &zero, sizeof(zero), 1) ||
           quantize_write_bytes(out, &zero, sizeof(zero), 1) ? -1 : 0;
}

int quantize_write_scalar_or_default(quantize_ctx *ctx, FILE *out, const char *const *names, size_t n_names, float default_value) {
    const weightmap_entry *entry = quantize_find_last_tensor(ctx, names, n_names);
    if (!entry) return quantize_write_bytes(out, &default_value, sizeof(default_value), 1);
    
    size_t expected_elements = 1;
    if (validate_entry(entry, entry ? entry->tensor_name : NULL, expected_elements) || open_entry_source(ctx, entry)) return -1;
    ...
    int res = 0;
    if (read_f32_range(ctx, entry, 0, 1, raw, f32) || quantize_write_bytes(out, f32, sizeof(float), 1)) res = -1;
