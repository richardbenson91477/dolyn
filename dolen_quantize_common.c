...
static int load_shard_metadata(safetensors_idx *idx, const char *filename) {
    ...
    for (size_t i = 0; i < root->data.object.count; i++) {
        ...
        if (strcmp(pair->key, "__metadata__") == 0) {
            continue;
        }
...
void quantize_group(qtensor *qt, const float *weights, int rows, int cols) {
    ...
    quantize_group_into(qt->q, qt->s, weights, rows, cols);
}
...
int load_safetensors_index(safetensors_idx *idx, const char *model_dir) {
    ...
    if (file_size < 0 || fseeko(f, 0, SEEK_SET) != 0) {
        fclose(f);
        return -1;
    }
...
const weightmap_entry *quantize_find_tensor(const quantize_ctx *ctx, const char *name) {
    for (size_t i = 0; i < ctx->index.n_entries; i++) {
        if (strcmp(ctx->index.entries[i].tensor_name, name) == 0) {
...
int quantize_write_f32_zeros(FILE *out, size_t elements) {
    size_t n = DOLEN_QUANTIZE_CHUNK_BYTES / sizeof(float);
    if (n == 0) {
        n = 1;
    }
    if (n > elements && elements > 0) {
        n = elements;
    }
    ...
    while (elements > 0) {
        size_t chunk = elements < n ? elements : n;
        if (quantize_write_bytes(out, zeros, sizeof(float), chunk)) {
            free(zeros);
            return -1;
        }
        elements -= chunk;
