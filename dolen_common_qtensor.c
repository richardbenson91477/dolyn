...
#pragma omp parallel for schedule(static)
        for (int32_t i = 0; i < d; i++) {
            ...
            const float *_w_row_s = _w_s + (size_t)i * n_groups;
            int8_t w_local[GROUP_SIZE]; // Stack buffer for vectorization

            for (int32_t g = 0; g < full_groups; g++) {
                int32_t offset = g * GROUP_SIZE;
                
                // 1. Unpack 64 Q6 weights into straight int8_t
                for (int32_t k = 0; k < GROUP_SIZE; k += 4) {
                    ...
                    w_local[k]     = (int8_t)((u0 ^ 0x20) - 0x20);
                    w_local[k + 1] = (int8_t)((u1 ^ 0x20) - 0x20);
                    w_local[k + 2] = (int8_t)((u2 ^ 0x20) - 0x20);
                    w_local[k + 3] = (int8_t)((u3 ^ 0x20) - 0x20);
                }
...
#pragma omp simd reduction(+ : acc)
            ...
            if (rem_start < n) {
                ...
                for (int32_t k = rem_start; k < n; k += 4) {
                    ...
                    if (k + 1 < n) acc += (int32_t)_x_q[k + 1] * w1;
                    if (k + 2 < n) acc += (int32_t)_x_q[k + 2] * w2;
                    if (k + 3 < n) acc += (int32_t)_x_q[k + 3] * w3;
                }
                val += (float)acc * _w_row_s[full_groups] * _x_s[full_groups];
            }
            _output[i] = val;
